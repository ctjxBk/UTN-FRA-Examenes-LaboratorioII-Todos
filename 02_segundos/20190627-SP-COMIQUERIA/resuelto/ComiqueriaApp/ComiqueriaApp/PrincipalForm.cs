...
namespace ComiqueriaApp
{
    public partial class PrincipalForm : Form
    {
        ...
        public PrincipalForm()
        {
            ...
            Dao.EventoMetodo += MostrarAcciones;
        }
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    // 4C - Realizar una baja física del producto seleccionado en la tabla de productos. 

                    try
                    {
                        Dao.Eliminar(this.productoSeleccionado.Codigo);
                    }
                    catch (Exception ex)
                    {
                        MostrarError(ex);
        ...
        private void ActualizarHora()
        {
            ...
            {
                Action medoto = this.ActualizarHora;
                this.Invoke(medoto);
            }
            else
            {
        ...
        internal static void MostrarError(string mensaje)
        {
            MessageBox.Show(mensaje, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        internal static void MostrarError(Exception ex)
        {
        ...
        private void MostrarAcciones(AccionesDB acciones)
        {
            this.ActualizarLista();
            ...
            switch(acciones)
            {
                case AccionesDB.Insert:
                    mensaje = "Se agrego exitosamente un producto a la base de datos";
                    break;
                case AccionesDB.Update:
                    mensaje = "Se actualizo exitosamente un producto a la base de datos";
                    break;
                case AccionesDB.Delete:
                    mensaje = "Se elimino exitosamente un producto en la base de datos";
                    break;
            }
            MessageBox.Show(mensaje, "Información Base de datos", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
